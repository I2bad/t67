...
window.DEMOS.unspoken = function (svg) {
  ...
  peers.forEach(function (p, i) {
    tl.to(p, { x: 560 + (i % 3) * 60, duration: 7 + (i % 4) * 0.6, ease: 'sine.inOut' }, i * 0.15);
  });

  // The caption lands mid-drift, once the pull is already visible
  tl.to(q('.quiet-say'), { opacity: 1, duration: 1 }, 3);
  ...
  tl.to(ball, { x: 330 + 420, duration: 6.5, ease: 'sine.inOut' }, 1.8)
    .to(ball, { rotation: 10, transformOrigin: '50% 50%', duration: 6.5, ease: 'sine.inOut' }, 1.8);
